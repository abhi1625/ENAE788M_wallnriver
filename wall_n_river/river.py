	...
	def run_pipeline(self):
		...
		while(not rospy.is_shutdown()):
			...
			if mission_hold_pos == True:
				# mission_hold_pos, mission_move_right = self.track_ob.mission(mission_hold_pos, self.vel, target_pos_init, 1, rate_mission)
				# rospy.sleep(3)
				print("reached altitude of "+str(h)+" m")

			status, error = self.bridge_detect(curr_frame)
			if final_flag == False:
				break
			if ((status == True) and ((error >= 150.0) or (error<=-150.0))):
				error = error*0.0001
				...
				self.track_ob.bebop_vel_pub.publish(self.vel)
			elif ((status == True) and ( -100 < error < 100.0) and (final_flag== True)):
				rospy.sleep(2)
				go_forward = np.array([3.0,self.track_ob.current_state[1],h])
				_, _= self.track_ob.mission(True, self.vel, go_forward, 1, rate)
				final_flag = False
			print("status error = ",status,error)
			# if status == True and  error <0.0:te
				# move right
			# 	target_pos_right = np.array([self.track_ob.current_state[0],-0.3 + self.track_ob.current_state[1],h])
			# 	# mission_move_right, _= self.track_ob.mission(True, self.vel, target_pos_right, 1, rate)
			# elif status == True and  error >0.0:
			# 	# move left
			# 	target_pos_left = np.array([0.0,0.3 + self.track_ob.current_state[1],h])
			# 	# mission_move_right,_= self.track_ob.mission(True, self.vel, target_pos_left, 1, rate)
			# elif status == True and  error ==0.0:
			# 	# hold
			# 	target_pos_hold = np.array([self.track_ob.current_state[0], self.track_ob.current_state[1],h])
			# 	# mission_move_right, _= self.track_ob.mission(True, self.vel, target_pos_hold, 1, rate)
